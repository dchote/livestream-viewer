...
The usual way to put live video on a screen attached to a Raspberry Pi is to run a full desktop session and point a browser at a page. That approach is expensive and fragile: Chromium will not use the Pi's hardware decoder for most stream types, a compositor and X/Wayland stack sit between the video and the panel, and a multi-stream grid quickly saturates the CPU. Digital signage products that take this route (Anthias and similar) are effectively playlist-driven browser kiosks and inherit all of the same costs.
...
- **Native rendering** — Decode with the platform's video hardware where available, upload frames to GPU textures, and composite with SDL3. No browser, no compositor, no desktop session. The application renders straight to KMS/DRM.
...
- **Single binary, single process** — One statically-configured Go binary that serves its own management UI and drives the display. Trivial to package, trivial to run as a service or a Home Assistant add-on.
- **Honest about hardware** — The Pi's decode capabilities changed significantly between Pi 4 and Pi 5. The application probes what is actually available and reports it, rather than silently falling back to software decode and dropping frames.
...
**Home Assistant add-on** — The repository can be added as a Home Assistant add-on repository for one-click install. The management UI is exposed through ingress; the add-on is granted access to the DRM render nodes so it can drive the attached display.

**Standalone** — A single binary, run as a systemd service on Raspberry Pi OS Lite. No desktop environment required; the application takes DRM master directly.

**Development** — Runs on a normal desktop Linux or macOS workstation against a windowed SDL output, so layouts and transitions can be developed without a Pi on the desk.
...
- **Makers and hobbyists** building an information wall, workshop display, or ambient screen
