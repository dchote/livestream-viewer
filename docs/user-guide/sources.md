...
- **Force hardware decode** — Leave this off for normal use. The wall already prefers hardware when the host supports the codec. Turn this on only if a macOS RTSP camera stays on software after Probe and you want to retry VideoToolbox. You cannot enable it together with **Force software decode**.
