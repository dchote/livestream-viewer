...
- Adding a source opens a StandardDialog whose fields change with the selected `kind`. The primary button is **Create** on add and **Save** when editing. YouTube shows a Terms of Service / yt-dlp notice, and a warning if `yt-dlp` is unavailable, from `GET /api/v1/system/info`. The page header shows a short **Decode:** capability line (H.264 / HEVC). **Buffer (seconds)** is the jitter buffer (`options.buffer_ms`); YouTube/HLS/DASH default to 4. **Force software decode** skips hardware. **Force hardware decode** retries VideoToolbox on RTSP after a failed probe; the two switches clear each other.
